...
// The rest of this file, and all of src/MFRC522_I2C.cpp, are byte-identical to
// upstream. Nothing below this block was modified; diff against the upstream
// commit named here to confirm.
//
// [MI2] kkloesener/MFRC522_I2C -- "MFR522 i2c Library to read NFC Tags with
//       Arduino or ESP8266/ESP32", library.properties version 1.0, upstream
//       states "Released into the public domain." in this file's own original
//       header (see immediately below).
//       https://github.com/kkloesener/MFRC522_I2C
//       commit 8152dddc93cf743397ac225e34bf268698326664 (2026-05-11).
//       Copied here 2026-08-19 from ~/src/unigeek-main's PlatformIO libdeps
//       cache, which is where UniGeek (one of this program's three authorized
//       donor firmwares) pulls it via its platformio.ini:64 lib_deps entry.
//
// WHY IT IS VENDORED RATHER THAN FETCHED. Phase 3 Task 4 originally reached
// this library through a `lib_extra_dirs` entry pointing at that libdeps CACHE
// in a different repository -- a path that is gitignored over there, wiped by
// any `pio pkg` operation in that project, and nonexistent on every other
// machine. A task review flagged it as a Critical build-reproducibility
// defect: nobody else could build this firmware, including this project's own
// owner after a cache clear. Vendoring the two files the build actually needs
// is the same remedy Task 3 applied for the equivalent problem in
// src/features/nfc/ws1850s_driver.cpp.
//
// WHAT USES IT. src/features/nfc/nfc_read.cpp's RFID2 tag-read path, for the
// PICC-level anticollision sequence only: PICC_IsNewCardPresent() ->
// PICC_ReadCardSerial() -> PICC_Select(), plus PICC_GetType()/PICC_HaltA()/
// PCD_StopCrypto1(). This project drives the chip's REGISTER level through its
// own src/features/nfc/ws1850s_driver.{h,cpp} (which is where PCD_Init()'s
// register programme is really applied); PCD_Init() is deliberately never
// called on the MFRC522_I2C instance itself. See nfc_read.cpp's call site for

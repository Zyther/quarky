 ...
 *
 * quarky/tab5 note (Task 6): this is LVGL 9.2.0's stock
 * lv_conf_template.h (fetched from
 * https://github.com/lvgl/lvgl/blob/v9.2.0/lv_conf_template.h to match
 * the `lvgl/lvgl@^9.2.0` pinned in platformio.ini) with the outer
 * `#if 0 ... #endif` "content enable" wrapper removed so the settings
 * below take effect, per the template's own instructions. Values were
 * left at their shipped defaults rather than hand-tuned, because the
 * defaults already satisfy this task's requirements:
 *   - LV_COLOR_DEPTH 16 matches DisplayTab5's RGB565 framebuffer
 *     (firmware/tab5/src/hal/display_tab5.h / idisplay.h flush() takes
 *     `const uint16_t *colors`).
 *   - LV_USE_LABEL / LV_USE_BUTTON / LV_USE_KEYBOARD are all `1` by
 *     default in the template -- covers this task's label+button test
 *     widget and the `lv_keyboard` a later UI task needs.
 * Lives in firmware/tab5/include/ so PlatformIO's default include path
 * satisfies platformio.ini's `-DLV_CONF_INCLUDE_SIMPLE` build flag
 * (`#include <lv_conf.h>` rather than a nested/relative path).
 *
 * One thing the template does NOT provide: LVGL 9's tick source. There
 * is no more `LV_TICK_CUSTOM` compile-time macro (removed vs LVGL 8) --
 * v9 instead expects the port to call `lv_tick_inc()` periodically or
 * register a callback via `lv_tick_set_cb()`. This project wires the
 * latter (`lv_tick_set_cb(millis)`) in ui/lvgl_port.cpp's
 * `lvgl_port_init()`; see that file for the citation
 * (lvgl/src/tick/lv_tick.h in the same v9.2.0 tag).
 */

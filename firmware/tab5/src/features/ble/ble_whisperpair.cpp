...
#include "ble_common.h" // ble_require_host_ready()
#include "ble_target_picker.h" // the shared scan -> tap-a-target -> hand-off
                                // screen this feature now starts with
...
static lv_obj_t *build_screen(const uint8_t addr_val[6], uint8_t addr_type) {
    ...
    // not a guarantee, and one that would have silently evaporated the day
    // anything else populated that list). It is now genuinely unreachable in
    // that state, since BleTargetPicker runs its own host-ready guard before
    // it can ever offer a target, but this guard stays: it is the one that
    // belongs to THIS screen. Placed after
...
void register_module() {
    // TARGET SELECTION -- resolved 2026-08-17 (whole-branch review finding I3,
    // deferred half). This detector is only meaningful against a Google Fast
    // Pair accessory (0xFE2C), and it used to connect to scan slot 0 of a
    // prior, separate BLE Scan screen run unconditionally. That mattered more
    // here than in ble_fastpair_exploit.cpp, because this file is a
    // VULNERABILITY DETECTOR and the cost of aiming it at the wrong device is
    // a result a hurried reader can mistake for a security finding. It now
    // scans for itself and the user taps the target -- see start() and
    // BleTargetPicker. See ble_fastpair_exploit.cpp's register_module() for
    // why extracting the picker into its OWN screen dissolved the
    // three-lifecycle teardown problem that made the earlier round defer this;
    // the same reasoning applies verbatim here, where the state machine is
    // larger still (subscription gating, probe window expiry) and the teardown
    // loop carries the same no-early-exit race documentation. Both are
    // untouched: this file's screen is still created with a target already in
    // hand and still has exactly one lifecycle.
    //
    // Still open: the picker lists every advertiser rather than ranking Fast
    // Pair candidates (BleClassify's labels could drive that). Until then the
    // mitigation is the verdict wording this file already uses and which must
    // be kept that way: a target with no 0xFE2C service reports "not
    // applicable to this target", never "patched" or "not vulnerable" -- see
    // svc_disc_cb()'s log_status() call.
...
// itself. build_screen() and everything it drives -- the connect, the 0xFE2C /
// KBP discovery, the real CCCD subscription, the ECDH/AES probe construction,
// the verdict wording, and the teardown loop that took two fix rounds to get
// right -- are unchanged by that fix: this feature's input changed, not its
// behaviour.
...
void start() {
    ...
    // BleScanFeature::first_device_addr() and push a "run BLE Scan first"
    // message screen when there was none; both are gone, since the user now
    // picks a real target here and the no-target case cannot occur.
    BleTargetPicker::start("WhisperPair -- pick a target", on_target_selected);
}

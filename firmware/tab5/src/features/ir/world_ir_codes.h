#pragma once
// #include added by this project -- the original file relies on Arduino.h
// (included transitively by whatever donor file included this one) for
// uint8_t/uint16_t; standalone stdint include added so this vendored file
// compiles regardless of include order.
#include <cstdint>

// ===========================================================================
// VENDORED THIRD-PARTY SOURCE -- provenance header added by this project.
// Everything below this block (other than the #pragma once/#include just
// above) is byte-identical to the copy it was taken from; diff against the
// path named below to confirm.
//
// [C1]  WORLD_IR_CODES.h -- the real "Gen3 TV-B-Gone" NA/EU power-off code
//       database (271 codes total), the same corpus shipped in actual
//       TV-B-Gone hardware keychains since 2009.
//         Copyright (c) Mitch Altman + Limor Fried 2009 (original ATtiny85v
//         firmware); ported to Arduino by Ken Shirriff; NA/EU universality
//         and further updates by Mitch Altman, ka1kjz, and Anton
//         Grimpelhuber (per the original header comment immediately below).
//         License: Creative Commons 2.5, Attribution & Share Alike (stated
//         in this same TV-B-Gone bundle's sibling file, TV-B-Gone.h,
//         which this data file is inseparable from) -- COPYLEFT-ish. This
//         is NOT project-original code and is NOT under this repository's
//         own terms.
//
//       COPIED FROM: ~/src/firmware/src/modules/ir/WORLD_IR_CODES.h
//       (Bruce, one of this program's three authorized donor firmwares --
//       see CLAUDE.md) on 2026-08-21, for Phase 3 Task 16
//       (features/ir/ir_tvbgone.cpp).
//
//       A SECOND donor checkout (~/src/unigeek-main/firmware/src/utils/ir/
//       TVBGoneData.h) carries the exact same 271-code NA/EU database
//       byte-for-byte (diff confirms lines 1-8522 are identical), which
//       independently cross-validates this is the same real, unmodified
//       upstream dataset in both forks -- not a fork-specific
//       modification. UniGeek's copy then APPENDS ~460 additional lines of
//       "universal power codes" (Samsung/Grundig/LG/Sony/etc., some using a
//       different RawIrCode struct with 32-bit timings) beyond this real
//       NA/EU set, under unclear separate authorship -- NOT vendored here;
//       a future task wanting that extended set should treat it as a
//       distinct addition requiring its own licensing check, not assume it
//       carries the same CC-BY-SA terms as this file.
//
//       ADAPTATION: none. This file is used as-is; the decode algorithm
//       (bit-packed `codes[]` index stream, `read_bits()`,
//       `freq_to_timerval()`) is reimplemented against this same layout in
//       ir_tvbgone.cpp, cross-validated against BOTH donors' own transmit
//       loops (Bruce's TV-B-Gone.cpp and UniGeek's IRUtil.cpp implement the
//       identical `rawData[k*2]=times[ti]*10, rawData[k*2+1]=times[ti+1]*10`
//       unpacking), rather than trusting either fork's own transmit-loop
//       code (which assumes an IRremoteESP8266-style `IRsend::sendRaw()`
//       this project does not have -- see ir_common.h for the real
//       RMT-peripheral-based replacement).
// ===========================================================================

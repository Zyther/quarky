...
- The external HY2.0 peripheral port (where NFC/RFID2/RF433 modules connect) was completely dead — same root cause class as the C6: a different bit on the same IO-expander, unasserted. Fixed by reusing the existing IO-expander driver. The connected NFC unit turned out to be an ST25R3916, not the PN532 originally assumed. (Phase 3 later proved the RFID2 unit is not a PN532 either — it is a WS1850S, MFRC522/PN512-family; see the Phase 3 spec's 2026-08-19 correction.)

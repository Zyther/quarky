...
- **RF433 GPIO pins remain unresolved, but the physical connector is now confirmed (2026-08-09).** A real RF433R unit uses the same HY2.0 PORT.A connector as NFC/RFID2 (swapped one at a time, not simultaneous) — the port isn't I2C-exclusive as Task 18 originally concluded. A real-hardware test driving GPIO 53/54 as digital inputs found no signal correlated with an actual button press on a 433MHz remote, with port power confirmed on — so those two pins are very likely not RF433R's real data line despite sharing the connector. Still left as an explicit, guarded `-1` TODO rather than a guessed value; next step needs a multimeter continuity check or better hardware documentation than what's publicly available (already exhausted — see `firmware/tab5/src/hal/rf433_gpio.cpp`'s citation list).

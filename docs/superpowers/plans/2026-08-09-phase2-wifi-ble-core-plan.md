...
> **DEFERRED (2026-08-12, real hardware result):** Task 6 was implemented, reviewed, and fixed through this plan's normal process (commits 6808416, 0bb9e85), but real-hardware verification found `esp_wifi_set_promiscuous_rx_cb()` and `esp_wifi_set_promiscuous(true)` both return `ESP_ERR_NOT_SUPPORTED` (262) immediately on the Tab5. A follow-up investigation (`task-6-promiscuous-investigation-report.md` in this plan's SDD workspace) disassembled the actual linked code and confirmed this is a hard architectural limitation, not a bug, config gap, or fixable version-skew issue: `esp_wifi_remote_set_promiscuous()`/`_rx_cb()` are Espressif's weak default stubs (`li a0,262; ret` — no RPC call, no SDIO round-trip), the same root cause as Task 4's `esp_wifi_80211_tx()` finding (`esp_wifi_remote_80211_tx()` is a byte-identical stub). Upstream esp-hosted has promiscuous support written but commented out server-side, with an open maintainer issue stating no near-term implementation plans. Per the project owner's explicit direction (same treatment as Task 4), this feature is **re-scoped to a Cardputer-ADV-affinity feature (native, non-hosted ESP32-S3 WiFi) or the planned ESP32-C5 sidecar**, and deferred to whichever future phase does that real hardware work — not attempted further on Tab5-native. The code (UI, ISR-to-main-task ring buffer, pcap writer, `IStorage` capture-file methods) is left merged as-is: it's correct and directly reusable once ported to a native-radio target, it just cannot capture anything on Tab5-native today. Task 7 (BLE Scan-Mode Spike + BLE Scan Feature) is next in this plan's actual execution order.

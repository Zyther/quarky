...
**Context:** ~~Port Bruce's `ESP-Amiibolink` (straightforward NTAG215 read/write per the spec, no crypto-attack complexity).~~ **Corrected 2026-08-21, wrong premise (same failure class as Task 3's PN532/MFRC522 mixup and Task 4's original RFAL/IRQ misunderstanding):** `ESP-Amiibolink` (`~/src/firmware/src/modules/rfid/amiibo.cpp`) does not read or write NTAG215 tags at all. Confirmed directly by fetching `bmorcelli/ESP-Amiibolink`'s real source (`gh api repos/bmorcelli/ESP-Amiibolink`): its own README says "An Arduino/ESP32 library for interacting with Amiibolink" and its `amiibolink.h` is a **NimBLE client** (Nordic UART Service UUIDs, `searchDevice()`/`connectToDevice()`/`cmdUploadDumpData()`) that uploads an already-dumped amiibo file to a **separate physical commercial device** called "Amiibolink" over BLE — it contains no NFC/ISO14443 tag I/O whatsoever, and this project has no such external hardware.

**Real replacement source, confirmed working against this exact chip family:** Bruce's own `~/src/firmware/src/modules/rfid/RFID2.cpp` (already this project's Task 3 precedent for the WS1850S/MFRC522-class driver) implements real, generic NFC-Forum-Type-2 (Ultralight/NTAG21x) read AND write directly against `MFRC522`-family objects, using exactly the two methods already vendored in this project's `lib/MFRC522_I2C/` (Task 4): `MIFARE_Read()` (`RFID2.cpp:428`, 4-page/16-byte reads via `PICC_CMD_MF_READ`, looping until a `STATUS_MIFARE_NACK` signals end-of-memory) and `MIFARE_Ultralight_Write()` (`RFID2.cpp:539`, single 4-byte page writes via `PICC_CMD_UL_WRITE`). Tag identity is read from the real Capability Container byte at page 3 (`RFID2.cpp:432-444`): CC=0x12 → NTAG213 (45 pages), CC=0x3E → **NTAG215 (135 pages)**, CC=0x6D → NTAG216 (231 pages) — no authentication needed (Ultralight/NTAG21x tags have no MIFARE Classic-style crypto), matching the plan's own "no crypto-attack complexity" expectation, just via the correct source. Port from `RFID2.cpp`'s real read/write/CC-detection logic instead, composing with `Ws1850sDriver`/`MFRC522_I2C` the same way `nfc_read.cpp`/`nfc_mifare_crack.cpp` already do (construct an own `MFRC522_I2C` instance, never call its `PCD_Init()`, rely on `Ws1850sDriver::init()` having programmed the chip). Module/file names (`nfc_amiibo.h/.cpp`, `NfcAmiibo`) are unchanged — the user-facing "Amiibo" framing (dump/restore/clone an NTAG215) is still accurate, only the donor source and underlying implementation change.
...
- [ ] **Step 2: Port the real NTAG21x read/write/CC-detection logic from `RFID2.cpp` (see corrected Context above), NOT `ESP-Amiibolink`**
...
- [x] **Step 4: PAUSE FOR HARDWARE, then verify against a real remote**

**PAUSE FOR HARDWARE:** confirm IR unit connected, project owner has a real IR remote ready to point at it. Wait for confirmation. **DONE, 2026-08-22 — real hardware confirmed, no bugs found:** four consecutive Start Capture / real-remote-button-press / result cycles all succeeded (5, 67, 6, 67 pulses across the runs), then several more (1, 3, 1, 3, 67 pulses) leading into a real Save to SD, which also succeeded (`/quarky/captures/ir/learn_155091.ir`, 67 pulses, confirmed via serial log). No crash, no reboot, no `State::kFailed` ("no signal detected") observed across any of these real attempts. This is real, positive confirmation of the exact thing the Task 17 review round's most severe finding was worried about (see the SDD ledger and `ir_learn.cpp`'s own `kIdleThresholdTicks` comment): the RMT RX idle-gap threshold does NOT complete prematurely before a real button press arrives -- every arm-then-press cycle correctly captured real data, matching Espressif's own documented "reception starts at the first level change" behavior rather than the reviewer's alternate (and ultimately unconfirmed) concern. The 8ms idle-threshold value itself also appears workable as-is: repeated captures of the same button consistently returned the same 67-pulse count, indicating clean, repeatable full-frame captures rather than truncation or merged-repeat artifacts.
